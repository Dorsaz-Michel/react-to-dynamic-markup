...
export default class Document {
    ...
    #metas: Promise<string>[] = [];
    #links: Promise<string>[] = [];
    #styles: Promise<string>[] = [];
    #headerScripts: Promise<string>[] = [];
    #bodyScripts: Promise<string>[] = [];
    #referencedHeaderScripts: {[key:string]: {attributes: {[p:string]: string|boolean}, functions: string[], DOMContentLoadedFunctions: string[]}} = {};
    #referencedBodyScripts: {[key:string]: {attributes: {[p:string]: string|boolean}, functions: string[], DOMContentLoadedFunctions: string[]}} = {};
     ...
     * </ul>
     */
    async renderToDynamicMarkup(reactNode: ReactNode) {

        const content = await this.#renderToString(reactNode);

        const referencedHeaderScripts: Promise<string>[] = [];
        const referencedBodyScripts: Promise<string>[] = [];
     ...
     */
    async #renderToString(reactNode: ReactNode): Promise<string> {
        ...
        // Array
        if (Symbol.iterator in (reactNode as Iterable<ReactNode>)) {
            let html = "";
            for (const node of reactNode as Iterable<ReactNode>) {
        ...
        let { type, props = {}} = reactNode as ReactElement;
    ...
    async #parseTag(tag: string, attributes: { [key: string]: any }): Promise<string> {
        ...
        Object.entries(attributes).forEach(([attribute, value]: [string, any]) => {
            if (attribute === "children")
                ...
                if (attribute.toLowerCase().startsWith('on') && typeof value === "function")
                    ...
                    html += ` style="${Object.entries(value).map(([styleAttribute, styleValue]: [string, any]) => `${styleAttribute}:${styleValue.toString()}`).join(';')}"`;
     ...
     */
    addMeta(attributes: {[p:string]: string|boolean}) {
     ...
     */
    addLink(attributes: {[p:string]: string|boolean}) {
     ...
     */
    addStyle(attributes: {[p:string]: string|boolean}, content?: string) {
        if (content)
            attributes.children = content;
     ...
     */
    addScript(location: "header"|"body", attributes: {[p:string]: string|boolean}, reference?: string) {
     ...
     *
     * NOTE: Function passed as content are converted to string.
     *
     * NOTE: Content added through this method with waitDomContentLoaded = true share the same DomContentLoaded listener
     *
     ...
     */
    addToScript(scriptReference:string, content: Function|string, waitDomContentLoaded?: boolean) {
